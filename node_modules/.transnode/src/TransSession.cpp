...
static uv_async_t event_notifier;
...
TransSession::pumpLogMessages()
{
    ...
    for (l=list; l!=NULL; l=l->next)
    {
        ...
        //uv_async_init(uv_default_loop(), &event_notifier, (uv_async_cb) this->events);
...
void pumpEvents2(uv_work_t *req) {
    
}

void pumpEventsDone(uv_work_t *req) {
    v8::HandleScope scope;
    //TRANSNODE_WORK_UNWRAP(session_events);
    session_events_req* r = (session_events_req*) req->data;
    

    const tr_evt_list * l;
    tr_evt_list * list = r->list;

    //v8::TryCatch try_catch;
    for (l=list; l!=NULL; l=l->next)
    {
        printf("pumpEvents2:%s\n", l->name);
        struct evbuffer * json = tr_bencToBuf(l->data, TR_FMT_JSON);
        Local<Value> argv[2];
        argv[0] = String::New(l->name);
        Handle<Value> json_str = String::New((const char*)evbuffer_pullup(json, -1), evbuffer_get_length(json));
        // JAJAJAJAJAJA
        Handle<Function> JSON_stringify = Handle<Function>::Cast(Context::GetCurrent()->Global()->Get(String::New("JSON"))->ToObject()->Get(String::New("parse")));
        argv[1] = JSON_stringify->Call(v8::Context::GetCurrent()->Global(), 1, &json_str);
        event_cb->Call(v8::Context::GetCurrent()->Global(), 2, argv);
        evbuffer_free (json);
...
/* DE MOMENTO CUELA PERO NO PUEDE SER ESTA KAKA DE ESTATICOS*/
TransSession::TransSession(string sessionConfigDir, Persistent<Function> events_cb)
{
    ...
    //uv_async_init(uv_default_loop(), &event_notifier, (uv_async_cb) events);
    //uv_queue_work(uv_default_loop(), &r->req, IDENTIFIER##_work, (uv_after_work_cb)IDENTIFIER##_after)
...
string TransSession::start()
{
	...
	tr_bencDictAddBool (&settings, TR_PREFS_KEY_RPC_ENABLED, false);
    ...
    //uv_idle_init(uv_default_loop(), &(this->uv_idle));
    //uv_idle_start(&(this->uv_idle), this->idle);


    //this->idle_req = new session_events_req;
    idle_req = new idle_events_req;
    idle_req->session = this;
    idle_req->req.data = idle_req;
    uv_timer_init(uv_default_loop(), &(idle_req->req));
    uv_timer_start(&(idle_req->req), (uv_timer_cb) idle_tn, 100, 1000);

    return response;
}

void idle_tn(uv_idle_t* req, int status)
{
    v8::HandleScope scope;
    TRANSNODE_WORK_UNWRAP(idle_events);
    //r->session->pumpLogMessages();
    //r->session->pumpEvents();
    session_events_req* rr = new session_events_req();
    rr->req.data = rr;
    //rr->cb = event_cb;
    //rr->list = NULL; //tr_getQueuedEvents ();
    rr->list = tr_getQueuedEvents ();

    uv_queue_work(uv_default_loop(), &rr->req, pumpEvents2, (uv_work_cb)pumpEventsDone);
}
...
void TransSession::emit(const char * evt, Handle<Object> data)
{
    printf("emit:%s\n", evt);
    v8::HandleScope scope;
    Local<Value> argv[2];
    argv[0] = String::New(evt);
    argv[1] = Local<Value>::New(data);
    printf("emitting %s", evt);
    scope.Close(event_cb->Call(v8::Context::GetCurrent()->Global(), 2, argv));
}
...
void TransSession::stop()
{
    ...
    uv_timer_stop(&(idle_req->req));
    if(this->watchdir)
        dtr_watchdir_free(this->watchdir);
    //tr_sessionSaveSettings(this->session, configDir, &settings);
    if(this->session)
        tr_sessionClose(this->session);
...
void init_offset(tr_torrent * tor, void * user_data) {
    tr_file_index_t offset = *((tr_file_index_t*) user_data);
    printf("init offset %d", offset);
    tr_torrentSetFilePriorities(tor, (tr_file_index_t*) &offset, 1, TR_PRI_HIGH);
    tr_file_index_t* f = tr_new0(tr_file_index_t, tor->info.fileCount-1);
    for (int i=0; i < tor->info.fileCount; ++i) {
        printf("here! %d\n", i);
        if(i != offset)
            //printf("DND±!± %d\n", i);
            f[i] = i;
    }
    tr_torrentSetFileDLs(tor, f, tor->info.fileCount, false);
    tr_free(f);
    tr_free(user_data);
}
...
int TransSession::addMagnet(string filePath, int offset)
{
    //HandleScope scope;
    printf("magnet: %d %s\n", offset, filePath.c_str());
    tr_ctor * ctor = tr_ctorNew (this->session);
    if ( tr_ctorSetMetainfoFromMagnetLink (ctor, filePath.c_str()) != 0 ) {
    	//Local<Object> evt_data = Object::New();
        //evt_data->Set(v8u::Symbol("magnet"), v8u::Str(filePath.c_str()));
        //emit("metadata:fail", evt_data);
        printf("emit: metadata:fail\n");
    ...
    if (err == TR_PARSE_ERR) {
        //emit("metadata:parseerr", evt_data);
        printf("emit: metadata:parseerr\n");
        return TORRENT_INVALID_FROMAT;
    }

    if(offset >= 0) {
        tr_file_index_t f = offset;
        tr_torrentSetFilePriorities(tor, &f, 1, TR_PRI_HIGH);
    }

    if (err == TR_PARSE_DUPLICATE) {
    	//emit("metadata:duplicate", evt_data);
        printf("emit: metadata:duplicate\n");
        return TORRENT_ALLREADY_EXISTS;
    }

    if(offset >= 0) {
        tr_file_index_t* cb_offset = tr_new(tr_file_index_t, 1);
        *cb_offset = offset;
        tr_torrentSetMetadataCallback(tor, init_offset, cb_offset);
    }

    //emit("metadata:torrent", evt_data);
    printf("emit: metadata:torrent\n");
    return 0;
}
...
TorrentsStat * TransSession::listTorrents()
{
        ...
        while ((tor = tr_torrentNext (this->session, tor)))
        {
            ...
            torrentStat[idx].fileStats = tr_torrentFiles(tor, &torrentStat[idx].numFiles);
...
int TransSession::startTorrent(int torrentId, bool bypass_queue)
{
    ...
    while ((tor = tr_torrentNext (this->session, tor)))
    {
    	...
    	{
            int activity = tr_torrentGetActivity (tor);
            switch (activity)
            {
                case TR_STATUS_SEED:
                case TR_STATUS_DOWNLOAD:
                ...
                case TR_STATUS_DOWNLOAD_WAIT:
                    if (!bypass_queue)
                        return activity; /* already queued */
                    break;
