...
static bool           eventQueuing = false;
static tr_evt_list *  eventQueue = NULL;
static tr_evt_list ** eventQueueTail = &eventQueue;
...
void
tr_freeEventList (tr_evt_list * list)
{
    tr_evt_list * next;
...
void pumpEvents(uv_work_t *req) {
    ...
    argv[0] = r->name;
    argv[1] = r->data;
    r->cb->Call(v8::Context::GetCurrent()->Global(), 2, argv);
    r->cb.Dispose();
    r->name.Dispose();
    r->data.Dispose();
    delete r;
...
void
tr_event (tr_session * session,
          tr_torrent * tor,
          const char * name,
          tr_benc* data)
{
    /* TODO: subscribe to events ... don't add them to the queue if they didn't happen */
    //printf("event(%s) %d\n", name, eventQueueCount);
    tr_evt_list * newevt;
    newevt = tr_new0 (tr_evt_list, 1);
    newevt->type = tor == NULL ? TR_EVT_SESS : TR_EVT_TOR;
    newevt->tor = tor;
    newevt->session = session;
    newevt->data = data;

    //newevt->when = tr_time ();
    newevt->name = tr_strdup (name);

    *eventQueueTail = newevt;
    eventQueueTail = &newevt->next;
    ++eventQueueCount;

    if (eventQueueCount > TR_MAX_EVT_LOG)
    {
        tr_evt_list * old = eventQueue;
        eventQueue = old->next;
        old->next = NULL;
        tr_freeEventList (old);

        --eventQueueCount;

        assert (eventQueueCount == TR_MAX_EVT_LOG);
    }
}

/*
void
tr_eventPropStr (const char * name,
                 const char * fmt,
                 ...)
{
    // add a property
...
void
tr_eventPropNum (const char * name,
                 const int val)
{
    // add a property
}
