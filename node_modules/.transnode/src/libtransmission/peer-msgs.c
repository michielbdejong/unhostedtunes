...
updateMetadataRequests (tr_peermsgs * msgs, time_t now)
{
        ...
        && tr_torrentGetNextMetadataRequest (msgs->torrent, now, &piece))
    {
        ...
        struct evbuffer * payload;
        struct evbuffer * out = msgs->outMessages;
        struct tr_torrent * tor = msgs->torrent;
        ...
        tr_bencDictAddInt (evt_data, "count", m->pieceCount);
        tr_bencDictAddInt (evt_data, "needed", m->piecesNeededCount);
        tr_event (NULL, tor, "metadata:progress", evt_data);

        //printf("tr_torrentGetNextMetadataRequest %d / %d\n", m->piecesNeededCount, m->pieceCount);
