...
tr_torrentSetMetadataSizeHint (tr_torrent * tor, int size)
{
    if (!tr_torrentHasMetadata (tor))
    {
        if (tor->incompleteMetadata == NULL)
        {
            ...
            tr_benc* evt_data = tr_new (struct tr_benc, 1);
            tr_bencInitDict (evt_data, 3);
            tr_bencDictAddStr (evt_data, "tor", tor->info.hashString);
            tr_bencDictAddInt (evt_data, "bytes", size);
            tr_bencDictAddInt (evt_data, "pieces", n);
            tr_event (NULL, tor, "metadata:downloading", evt_data);
            ...
            if ((metainfoParsed = !err))
            {
                ...
                if (!tr_bencLoadFile (&newMetainfo, TR_FMT_BENC, path))
                {
                    ...
                    if (success)
                    {
                        ...
                        if(tr_bencDictFindDict (&newMetainfo, "info", &info)) {
                            tr_benc * files;
                            if(tr_bencDictFindList(info, "files", &files)) {
                                tr_bencListCopy (tr_bencDictAddList (evt_data, "files", tr_bencListSize (files)), files);
                            ...
                            if(tr_bencDictFindStr(info, "name", &name)) {
                                ...
                                //tr_bencGetStr(find_benc, &val);
                                tr_bencDictAddStr(evt_data, "name", name);
                            }
                        }
                        
                        tr_event (NULL, tor, "metadata", evt_data);
...
tr_torrentGetNextMetadataRequest (tr_torrent * tor, time_t now, int * setme_piece)
{
    ...
    struct tr_incomplete_metadata * m;
    //printf("tr_torrentGetNextMetadataRequest\n");
